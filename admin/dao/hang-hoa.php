<?php
    ...
    function hang_hoa_insert($ten_hh,$hinh,$don_gia,$giam_gia,$mo_ta,$ma_loai,$size,$maMau,$so_luong){//hàm thêm hàng hóa
        $sql = "INSERT INTO hang_hoa(ten_hh,hinh,don_gia,giam_gia,mo_ta,ma_loai,maSize,maMau,so_luong) VALUES (?,?,?,?,?,?,?,?,?)";//chèn vào bảng hh các thông tin tương ứng
        pdo_execute($sql,$ten_hh,$hinh,$don_gia,$giam_gia,$mo_ta,$ma_loai,$size,$maMau,$so_luong);
    }

    function hang_hoa_update($ma_hh, $ten_hh, $hinh, $don_gia, $giam_gia, $mo_ta, $ma_loai, $size, $mauSac,$so_luong){//sửa sản phẩm
        $sql = "UPDATE hang_hoa SET ten_hh=?, hinh=?, don_gia=?, giam_gia=?, mo_ta=?, ma_loai=?, maSize=?, maMau=?,so_luong=? WHERE ma_hh=?";//cập nhật tt sp theo mã sp
        pdo_execute($sql, $ten_hh, $hinh, $don_gia, $giam_gia, $mo_ta, $ma_loai, $size, $mauSac, $ma_hh,$so_luong);
        }
